...
const NotificationCard = ({ icon: Icon, time, title, description }: NotificationCardProp) => {
  return (
    <div className="border border-grey-200 rounded-sm p-4 mb-3 shadow-sm even:bg-grey_100 cursor-pointer">
      <div className="flex gap-6 items-center">
        <div>
          <Icon className="text-xl text-shorttiee-primary" />
        </div>
        <div className="space-y-2">
          <p className="text-xs text-grey-300 font-medium">{time}</p>
          <h3 className="text-base text-shorttiee-primary font-semibold">{title}</h3>
          <p className="text-md text-grey-300 font-medium">{description}</p>
        </div>
      </div>
    </div>
  );
      ...
      <div className="space-y-4">
        {notifcationArr.length === 0 ? (
          <div className="flex flex-col justify-center items-center mx-auto h-full space-y-8">
            <div className="p-4">
              <BellRing className="size-12 text-shorttiee-primary animate-ping" />
            </div>
            <div>
              <h3 className="text-xl text-shorttiee-primary font-medium">
